...
# relu_like_act = 'relu' or 'swish'
def CreateReaderModel(relu_like_act):
    inputs = layers.Input((28, 28, 1), dtype=REAL_TYPE)
    pool_stages = (0, 2)
    [x, flags_axis1, flags_axis2] = libinv.OptimalShiftFront2D(inputs, pool_stages, (2, 2))
    x = libinv.CircConv2D(5, (3, 3), activation=relu_like_act)(x)
    x = libinv.CircConv2D(5, (3, 3), activation=relu_like_act)(x)
    x = libinv.DirectDownSample2D(x)
    x = libinv.CircConv2D(5, (3, 3), activation=relu_like_act)(x)
    x = libinv.CircConv2D(5, (3, 3), activation=relu_like_act)(x)
    x = libinv.DirectDownSample2D(x)
    x = libinv.CircConv2D(10, (3, 3), activation=relu_like_act)(x)
    x = layers.GlobalAveragePooling2D(dtype=REAL_TYPE)(x)
    outputs = layers.Activation('softmax', dtype=REAL_TYPE)(x)
    return keras.Model(inputs, outputs)
...
denoised_train = denoiser.predict(noisy_train)
denoised_test = denoiser.predict(noisy_test)

reader = CreateReaderModel('swish')
reader.compile('adam', loss='categorical_crossentropy', metrics=['accuracy',])
reader.summary()
keras.utils.plot_model(reader, to_file='Reader.png', show_shapes=True)

history3 = reader.fit( x=denoised_train, y=train_read,
                       epochs=epochs_reader,
                       batch_size=128, shuffle=True,
                       validation_data=[denoised_test, test_read]
                     )
reader.save('Reader')

denoised_reader_loss_list = history3.history['loss']
denoised_reader_val_loss_list = history3.history['val_loss']
...
score = reader.evaluate(denoised_test, test_read)
...
score = reader.evaluate(noisy_test, test_read)
accuracy_file.write('reader on noisy_test: loss = ' + str(score[0]) + '\n')
accuracy_file.write('reader on noisy_test: accuracy = ' + str(score[1]) + '\n')
accuracy_file.close()

history4 = reader.fit( x=noisy_train, y=train_read,
                       epochs=epochs_reader,
                       batch_size=128, shuffle=True,
                       validation_data=[noisy_test, test_read]
                     )

noisy_reader_loss_list = history4.history['loss']
noisy_reader_val_loss_list = history4.history['val_loss']
