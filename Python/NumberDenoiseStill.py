...
def CreateDenoiserModel(relu_like_act):
    x = inputs = layers.Input(shape=(28, 28, 1))
    for shift0 in range(0, shifts[0]):
        for shift1 in range(0, shifts[1]):
            # Shift forward
            y = tf.roll(x, shift0, axis=1)
            y = tf.roll(y, shift1, axis=2)
            # Encoder
            y = libinv.CircConv2D(n_code_filters, (3, 3), activation=relu_like_act)(y)
            y = libinv.CircMaxPool2D(y, relu_like_act)
            y = libinv.CircConv2D(n_code_filters, (3, 3), activation=relu_like_act)(y)
            y = libinv.CircMaxPool2D(y, relu_like_act)
            # Decoder
            y = libinv.CircConv2DTrans(n_code_filters, (3, 3), strides=2, activation=relu_like_act)(y)
            y = libinv.CircConv2DTrans(n_code_filters, (3, 3), strides=2, activation=relu_like_act)(y)
            y = libinv.CircConv2D(1, (3, 3))(y)
            # Shift backward
            y = tf.roll(y, -shift1, axis=2)
            y = tf.roll(y, -shift0, axis=1)
            if shift0 == 0 and shift1 == 0:
                z = y
            else:
                z = tf.math.add(z, y)
    # Final sigmoid
    outputs = layers.Activation('sigmoid', dtype=REAL_TYPE)(z)
