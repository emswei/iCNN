...
funcToTest = @(x) WienerPade(x, imageScale, sigmaPix, USF) ;
inImage = LoadData() ;
...
daspect([1 1]) ;
print('-dpng', ['TestWienerPadeUp', num2str(USF), 'Spectrum']) ;

figure ; imagesc(outImage) ;
set(gca, 'linewidth', 1, 'fontsize', 11) ;
daspect([1 1]) ;
print('-dpng', ['TestWienerPadeUp', num2str(USF), 'Image']) ;

[Ny, Nx] = size(outImage) ;
Ly = 1 + floor(50 * USF + 0.5) ;
figure ; plot(1:Nx, outImage(Ly, :)) ; grid ;
print('-dpng', ['TestWienerPadeUp', num2str(USF), 'Row', num2str(Ly)]) ;